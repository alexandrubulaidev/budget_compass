...
class AppExpansionPanelList extends StatefulWidget {
  ...
  ///
  /// For AppExpansionPanelList, the callback needs to setState when it's
  /// notified
  ...
  /// Defines color for the divider when [AppExpansionPanel.isExpanded]
  /// is false.
  ///
...
class _AppExpansionPanelListState extends State<AppExpansionPanelList> {
  ...
  @override
  void didUpdateWidget(final AppExpansionPanelList oldWidget) {
    ...
    if (widget._allowOnlyOnePanelOpen) {
      ...
      // If the previous widget was non-radio AppExpansionPanelList,
      //initialize the
  ...
  ExpansionPanelRadio? searchPanelByValue(
    final List<ExpansionPanelRadio> panels,
    final Object? value,
  ) {
    ...
    for (var index = 0; index < widget.children.length; index += 1) {
      ...
      Widget? expandIconContainer = ExpandIcon(
        ...
        onPressed: !child.canTapOnHeader
            ? (final isExpanded) => _handlePressed(isExpanded, index)
            : null,
      );
